...
description: "Researches the state of a (chip vendor list, framework, feature) target in open-source AI inference/training, e.g. 'NVIDIA + vLLM + EP' or '[AMD, NVIDIA] + vLLM + EP'. Produces dashboard-ready per-topic JSON files plus a synthesized Markdown report."
compatibility:
  - claude-code
  - cursor
  - codex
...
- `feature_keywords` — **not a session-intake input.** The Phase 4 plot role prompts the user for this list at runtime (case-insensitive substring keywords used to filter PRs/issues to feature-relevant rows; e.g. for `feature: EP` the user might supply `["EP", "expert parallel", "expert-parallel", "EPLB"]`). Documented here as a non-input so a reader looking for it knows where it lives. The bare `feature` tag alone is rejected as too generic — see Procedure step 0 in `agents/plot_ecosystem_activity.md`.
...
| **C8** | Phase 4 feature-activity raw-rows pattern — one `execute_sql` per `(repo, metric)` over the full `search_window` with a feature-keyword OR-chain in the `WHERE`, returning RAW rows (`number, title, labels, bucket_at, repo`); the plot agent classifies vendor client-side. Server-side `GROUP BY` aggregation is forbidden (see hard rule 11 in `agents/plot_ecosystem_activity.md`). Net change: `O(months × repos × metrics)` `gh search` round-trips → `O(repos × metrics)` `execute_sql` round-trips (plus optional half-window splits when the >1000-row cap is hit). Same CSV schema `month,repo,vendor_group,count`; `BOTH` / `NEITHER` literals are no longer emitted. |
...
| **C8.3** | Phase 4 feature-keyword filter — sourced from the user prompt at Phase 4 time (no `feature_scope_map.md`, no silent fallback to `[feature]` alone). The role pre-validates length ≥ 2 and per-keyword length ≥ 3 chars; see Procedure step 0 in `agents/plot_ecosystem_activity.md`. |
...
### Phase 4 — Feature activity plot (main agent + plot sub-agent)

Phase 4 runs **after Phase 3 has written `REPORT.md`** and **before the Phase-6 hand-off paragraph**. Unlike Phases 1–3, Phase 4 is **best-effort feature-specific activity context** — it is NOT covered by the three-stage monitor audit (no `monitor_*` re-sample). Its purpose is to give the report a cross-vendor monthly activity series for the run's feature `{feature}` (e.g. EP) across the framework repos at v1 (`vllm-project/vllm` + `sgl-project/sglang`). The chart is one line per `(repo, vendor)`, restricted to PRs/issues whose title or labels match BOTH a feature keyword (resolved at Phase 4 time from the user) AND a vendor keyword (from `scope/chip_scope_map.md`). The legacy directory name `ecosystem_plots/` is kept for back-compat with downstream tools and the Phase-3 optimistic section.
...
4. **Resolve the feature keyword filter.** The main agent does NOT pre-bind `feature_keywords` from anywhere — there is no `feature_scope_map.md` and `scope.json.feature` is just a short tag (e.g. `EP`). The plot role itself prompts the user for the keyword list at runtime (Procedure step 0 in `agents/plot_ecosystem_activity.md`) the FIRST time Phase 4 runs in a session; if the user supplies a list when the main agent asks the metric question above, the main agent passes it through as a pre-bound `{feature_keywords}` so the plot role does not re-prompt.
5. **Spawn the plot role.** In parallel sub-agent mode, spawn one `plot_ecosystem_activity` worker per chosen metric in a single message; in serial fallback mode, run the role once per metric in the main agent. Each invocation receives the prompt template at `agents/plot_ecosystem_activity.md` with `{metric}`, `{repos}`, `{date_range}`, `{vendor_groups}`, `{chip_scope_map_path}`, `{feature}`, `{feature_keywords}`, and `{session_out_dir}` bound (where `{session_out_dir} = out_dir/`, `{feature}` from `scope.json.feature`, `{feature_keywords}` either pre-bound from step 4 or empty so the role re-prompts), and writes exactly three artifacts:
...
6. **Wait for all spawned plot roles to finish.** If any role reports a hard failure (e.g. `gh search` repeatedly rate-limited), record the failure in the hand-off but do not block Phase 6 — Phase 4 is best-effort.
...
- Vendor classification keywords MUST be derived from `scope/chip_scope_map.md` at runtime (single source of truth; same file Phase 0 reads). Feature filter keywords MUST come from the user prompt at Phase 4 time (no `feature_scope_map.md`, no silent fallback to `[feature]` alone). See hard rule 3 in `agents/plot_ecosystem_activity.md`.
...
- The MCP `execute_sql` query MUST return raw rows (one per matching PR/issue). Server-side `GROUP BY` / `COUNT(*)` aggregation is forbidden — see hard rule 11 in `agents/plot_ecosystem_activity.md` for the rationale (an earlier run aggregated server-side and ended up bypassing the canonical renderer).
...
| `agents/plot_ecosystem_activity.md` | Phase 4 feature-activity plot sub-agent prompt — produces one CSV + PNG + methods note per metric. Best-effort context outside the three-stage audit trail; vendor keywords are derived from `scope/chip_scope_map.md` at runtime, feature keywords from a runtime user prompt (Procedure step 0). File name kept for back-compat. |
| `scripts/plot_ecosystem_activity.py` | Phase 4 CSV → PNG renderer (one line per `(repo, vendor)`). Pure file-in / file-out so the chart can be regenerated independently of the sub-agent. Pass `--feature` to interpolate the run's feature into the title. |

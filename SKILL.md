...
description: "Researches the state of a (chip vendor, framework, feature) triple in open-source AI inference/training — e.g. 'NVIDIA + vLLM + EP'. Produces dashboard-ready per-topic JSON files plus a synthesized Markdown report."
compatibility:
  - claude-code
  - cursor
  - codex
...
- `ecosystem_plot_metric` — optional. One of `merged_prs`, `opened_issues`, `closed_issues`, `all`, or `skip`. Drives the Phase 4 ecosystem activity plot. If absent, the main agent asks the user once after Phase 3 (default if the user accepts defaults: `merged_prs`).
...
- `ecosystem_plot_vendor_groups` — optional. List of vendor names to classify against (each MUST match a `## <vendor>` heading in `scope/chip_scope_map.md`). Default: inherits `chip_list` (per C8.2; which itself defaults to `["AMD", "NVIDIA"]` when neither is supplied).
...
The `mcp_args` and `sql_predicate_*` fields mirror the currently probed
`signals-service` schema. The Stage-1.5 appendix
`sources/signals_service_discovered.md` remains the source of truth for the live
MCP server's date-filter support and column names; if a future host exposes a
different schema, refresh that appendix first.
...
- `MCP_SQL_USABLE` — controls the Phase 4 raw-rows pattern (per C8) used by the Phase 4 SQL raw-rows path. True iff `MCP_DETAIL_USABLE` is True AND the Stage-1.5 probe succeeded against `execute_sql()` for a Phase 4–shaped query. Defaults to **False** if `db_health()` errors.
...
Resolved verdict on the registered host: `MCP_DETAIL_USABLE: true`, `MCP_SQL_USABLE: true` (per `sources/signals_service_discovered.md`). The gating contract is unchanged — the per-PR MCP path (used by Phase 1 researchers and Phase 2 monitors) and the Phase 4 SQL raw-rows path (used by the plot role) still branch on these flags so a different host with the server unreachable falls back to `gh` automatically.
...
|---|---|
| **C1** | Canonical phase order — Phase 4 (ecosystem plot) ALWAYS runs before Phase 5 (comparison synthesis) so the comparison report can embed the plots. See **Hard rule 8** for the full ordering and Phase-5 skip condition. |
...
| **C6** | Two independent MCP capability flags — `MCP_DETAIL_USABLE` (controls the per-PR `search_signals` + `get_signal_detail` path) and `MCP_SQL_USABLE` (controls the Phase 4 SQL raw-rows path). Both default to **False** if `db_health()` errors. Folded into a single `data_source` variable: `mcp_first` when either is True, `gh_only` when both are False. |
| **C7** | `signals-service` MCP server is referenced by its **registered name only** in committed docs; the URL lives in the host's MCP config (per-host paths in the **Per-host MCP setup** table in `sources/source_playbook.md`). The only file in this repo that may contain the URL is `sources/signals_service_discovered.md` (Stage-1.5 probe output), and only as an environment-specific recorded probe target. |
| **C8** | Phase 4 ecosystem-plot raw-rows pattern — one `execute_sql` per `(repo, metric)` over the full `search_window`, returning RAW rows (`number, title, labels, bucket_at, repo`); the plot agent classifies client-side. Net change: `O(months × repos × metrics)` `gh search` round-trips → `O(repos × metrics)` `execute_sql` round-trips. Same CSV schema `month,repo,vendor_group,count`. |
| **C8.1** | `ecosystem_plots/` lives at top-level `out_dir/` (NOT under any vendor folder). Phase 4 runs ONCE per session. |
...
8. **Canonical phase order (per C1).** The main agent MUST execute phases in this exact order: `Phase 0 → Phase 1 (1a + 1b) → Phase 2 → Phase 3 → Phase 4 → Phase 5 (only when len(chip_list) == 2) → Phase 6 (hand-off)`. Phase 4 (ecosystem plot) ALWAYS runs before Phase 5 (comparison synthesis) so the comparison template can embed the ecosystem plots via `[render_if_present ecosystem_plots]`. Phase 5 is skipped entirely when `len(chip_list) == 1`.
...
2. For each `vendor` in `chip_list`: in parallel sub-agent mode, spawn one `researcher` worker per default topic in a single message. In serial fallback mode, run those researcher roles one at a time in the main agent. Each researcher's prompt is built from `agents/researcher.md` + the per-topic spec from `default_topics.md` + the resolved per-vendor scope + the source playbook + the target output path `{vendor_out_dir}/topics/{topic_name}.json`. NOTE: do NOT run a researcher for `external_repo_dependencies` here — it is produced by an analyzer in Phase 1b, not a generic researcher.
...
2. Read `templates/REPORT_template.md` and populate it with the per-topic JSON contents from `out_dir/{vendor}/topics/`:
   ...
   - **`## Ecosystem Activity Context`** — emitted iff the session-wide setting `ecosystem_plot_metric != skip` (known at Phase 3 time). The synthesizer enumerates one `plot` block per expanded metric and resolves each `plot.png_relpath` / `plot.csv_relpath` / `plot.methods_relpath` to `../ecosystem_plots/<metric>_by_vendor.png` (etc.) so the path is correct from `out_dir/{vendor}/REPORT.md`. The referenced files are written by Phase 4; this section is intentionally optimistic.
...
### Phase 4 — Ecosystem activity plot (main agent + plot sub-agent)

Phase 4 runs **after Phase 3 has written `REPORT.md`** and **before the Phase-6 hand-off paragraph**. Unlike Phases 1–3, Phase 4 is **best-effort ecosystem context** — it is NOT covered by the three-stage monitor audit (no `monitor_*` re-sample), and it is decoupled from the run's `(chip, framework, feature)` triple. Its purpose is to give the report a cross-vendor activity baseline for the broader ecosystem (`vllm-project/vllm` + `sgl-project/sglang` at v1).

**Ordering contract (per C1).** Phase 3 has already written each per-vendor `out_dir/{vendor}/REPORT.md` with an optimistic `## Ecosystem Activity Context` section referencing `../ecosystem_plots/<metric>_by_vendor.png` (etc.). Phase 4 writes those referenced files now; the plot role does NOT modify any `REPORT.md`. Phase 5 (comparison synthesis, when `len(chip_list) == 2`) re-uses the same Phase 4 artifacts via its own `[render_if_present ecosystem_plots]` block referencing `ecosystem_plots/<metric>_by_vendor.png` (relative to `out_dir/`). When `ecosystem_plot_metric == skip`, Phase 4 writes no artifacts; per-vendor REPORT.md and the comparison report omit the `## Ecosystem Activity Context` section entirely because their `[render_if_present ecosystem_plots]` blocks resolve to empty.
...
1. **Resolve the metric set.**
   ...
   - Otherwise, the main agent asks the user once (using the host's interactive prompt — `AskQuestion` in Cursor, `read` in Codex serial fallback) which metric(s) to plot. If the user picks `skip`, do NOT create `out_dir/ecosystem_plots/`. Continue to Phase 5 when `len(chip_list) == 2`; otherwise advance directly to Phase 6. (Phase 4 being skipped does NOT change the Phase 5 gate per hard rule 8 — the gate is purely `len(chip_list) == 2`. When the gate is satisfied, the Phase 5 comparison report's `[render_if_present ecosystem_plots]` block simply resolves to empty.)
2. **Resolve the window** from `ecosystem_plot_window` (PRIMARY default per C2: the session-wide `search_window` resolved at Phase 0 — concretely `{search_window.start_month}..{search_window.end_month}`. SECONDARY fallback: trailing 24 full months ending the previous calendar month-end, used only when `search_window` is unbound — i.e. Phase 4 is invoked outside the normal Phase-0 flow) and the **repos** from `ecosystem_plot_repos` (default `["vllm-project/vllm", "sgl-project/sglang"]`).
3. **Resolve the vendor groups** from `ecosystem_plot_vendor_groups` (default `["AMD", "NVIDIA"]`). The plot role itself reads the canonical `scope/chip_scope_map.md` at runtime to derive the keyword set per vendor — it does NOT consume `out_dir/scope.json` (which only resolves the run's single chip) and there is NO parallel `ecosystem_activity_keywords.md` file.
4. **Spawn the plot role.** In parallel sub-agent mode, spawn one `plot_ecosystem_activity` worker per chosen metric in a single message; in serial fallback mode, run the role once per metric in the main agent. Each invocation receives the prompt template at `agents/plot_ecosystem_activity.md` with `{metric}`, `{repos}`, `{date_range}`, `{vendor_groups}`, `{chip_scope_map_path}`, and `{session_out_dir}` bound (where `{session_out_dir} = out_dir/`), and writes exactly three artifacts:
...
5. **Wait for all spawned plot roles to finish.** If any role reports a hard failure (e.g. `gh search` repeatedly rate-limited), record the failure in the hand-off but do not block Phase 6 — Phase 4 is best-effort.
...
- Vendor classification keywords MUST be derived from `scope/chip_scope_map.md` at runtime (single source of truth; same file Phase 0 reads).
- Title + labels only for classification (no PR/issue body fetch in the bulk pass — see `agents/plot_ecosystem_activity.md` for rationale).
...
| `sources/source_playbook.md` | `mcp:signals` (PRIMARY) + `gh` (DOCUMENTED FALLBACK) + WebFetch / WebSearch / MLPerf / InferenceX recipes |
| `sources/signals_service_discovered.md` | Stage-1.5 MCP probe output. Records the live `signals-service` server's resolved canonical strings (`signal_id_format`, column names, date-filter support), the two capability flags (`MCP_DETAIL_USABLE` / `MCP_SQL_USABLE`), and the Phase 4 SQL template. The only file in this repo that may contain the MCP server URL (per C7) — and only as an environment-specific probe target. |
...
| `agents/plot_ecosystem_activity.md` | Phase 4 ecosystem activity plot sub-agent prompt — produces one CSV + PNG + methods note per metric. Best-effort context outside the three-stage audit trail; vendor keywords are derived from `scope/chip_scope_map.md` at runtime. |
| `scripts/plot_ecosystem_activity.py` | Phase 4 CSV → PNG renderer. Pure file-in / file-out so the chart can be regenerated independently of the sub-agent. |
| `templates/REPORT_template.md` | Synthesized per-vendor report skeleton |
| `templates/COMPARISON_REPORT_template.md` | Side-by-side comparison report skeleton rendered by Phase 5 when `len(chip_list) == 2`. Uses `{{vendor_a}}` / `{{vendor_b}}` placeholders and a `[render_if_present ecosystem_plots]` block populated by Phase 4 artifacts. |
